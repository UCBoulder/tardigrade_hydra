...
namespace tardigradeHydra {
    ...
    namespace micromorphicDruckerPragerPlasticityOptimization {
        ...
        void residual::setStateVariableResiduals() {
            ...
            const floatVector slackVariables(get_plasticStateVariables()->begin() + numPlasticMultipliers +
            ...
            for (auto y = microGradientYield->begin(); y != microGradientYield->end(); y++) {
                ...
                (*residual.value)[row0 + index + 2] = -(*y) - slackVariables[index + 2];
            }
        }
        ...
        void residual::setStateVariableJacobians() {
            ...
            for (unsigned int i = 0; i < numPlasticMultipliers; i++) {
                ...
                (*jacobian.value)[numUnknowns * (i + row0) + i + numPlasticMultipliers + numPlasticStrainLikeISVs +
                                  offset]                                = (*plasticMultipliers)[i];
            }
        ...
        void residual::suggestInitialIterateValues(std::vector<unsigned int> &indices, std::vector<floatType> &values) {
            ...
            if ((*get_macroYield()) > 0) {
                ...
                floatType delta =
                    std::fmax((*get_plasticMultipliers())[0],
                              hydra->hydra_configuration.tolr * (*get_macroYield()) + hydra->hydra_configuration.tola);
            ...
            if ((*get_microYield()) > 0) {
                // Perturb the plastic multiplier
                floatType delta =
                    std::fmax((*get_plasticMultipliers())[1],
                              hydra->hydra_configuration.tolr * (*get_microYield()) + hydra->hydra_configuration.tola);
            ...
            for (unsigned int i = 0; i < dimension; i++) {
                ...
                if ((*get_microGradientYield())[i] > 0) {
                    // Perturb the plastic multiplier
                    floatType delta = std::fmax((*get_plasticMultipliers())[i + 2],
                                                hydra->hydra_configuration.tolr * (*get_microGradientYield())[i] +
