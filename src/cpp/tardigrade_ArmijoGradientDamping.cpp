...
namespace tardigradeHydra {
     ...
     * Reset the dampening
     */
    void ArmijoGradientDamping::reset() {
        resetLSIteration();
     ...
     */
    void ArmijoGradientDamping::setLSAlpha(const floatType &value) { _lsAlpha = value; }
     ...
     */
    void ArmijoGradientDamping::setMaxLSIterations(const unsigned int &value) { _maxLSIterations = value; }
    ...
    bool ArmijoGradientDamping::checkLSConvergence() {
        TARDIGRADE_ERROR_TOOLS_CHECK(step != nullptr, "The step has not been defined");
        if (tardigradeVectorTools::l2norm(*getResidual()) <
            step->getToleranceScaleFactor() * (1 - getLSAlpha()) * (*getLSResidualNorm())) {
            return true;
        }
     ...
     */
    bool ArmijoGradientDamping::checkLSIteration() { return getLSIteration() < getMaxLSIterations(); }
    ...
    //
    //        *residualNorm.value = std::inner_product( std::begin( *residual ), std::end( *residual ), std::begin(
    ...
    //
    //            throw std::runtime_error( "The base residual norm must be set with set_baseResidualNorm before it can
    //            be called" );
    //
    //        }
    //
    ...
    //
    //            throw std::runtime_error( "The base residual norm must be set with set_dbaseResidualNormdX before it
    //            can be called" );
    //
    //        }
    //
