...
namespace tardigradeHydra {
    ...
    void HyperelasticBase::compute_dI1dFe(const bool isPrevious, dI1dFe_iter dI1dFe_begin, dI1dFe_iter dI1dFe_end) {
        ...
        TARDIGRADE_ERROR_TOOLS_CHECK((unsigned int)(dI1dFe_end - dI1dFe_begin) == dimension * dimension,
                                     "dI1dFe has a size of " +
                                         std::to_string((unsigned int)(dI1dFe_end - dI1dFe_begin)) +
                                         " but it should have a size of " + std::to_string(dimension * dimension))
    ...
    void HyperelasticBase::compute_d2I1dFe2(d2I1dFe2_iter d2I1dFe2_begin, d2I1dFe2_iter d2I1dFe2_end) {
        ...
        for (unsigned int ij = 0; ij < dimension * dimension; ++ij) {
            *(d2I1dFe2_begin + dimension * dimension * ij + ij) += 2;
        }
    }
    ...
    void HyperelasticBase::compute_right_cauchy_green_deformation_tensor(const bool isPrevious, C_iter C_begin,
        ...
        TARDIGRADE_ERROR_TOOLS_CHECK((unsigned int)(std::end(*Fe) - std::begin(*Fe)) == dimension * dimension,
                                     "The storage array for F is of size " +
                                         std::to_string((unsigned int)(std::end(*Fe) - std::begin(*Fe))) +
                                         " but must be of size " + std::to_string(dimension * dimension))

        TARDIGRADE_ERROR_TOOLS_CHECK((unsigned int)(C_end - C_begin) == dimension * dimension,
        ...
        for (unsigned int i = 0; i < dimension; ++i) {
            for (unsigned int I = 0; I < dimension; ++I) {
                for (unsigned int J = 0; J < dimension; ++J) {
                    *(C_begin + dimension * I + J) += (*Fe)[dimension * i + I] * (*Fe)[dimension * i + J];
                }
            }
        }
    }
    ...
    T HyperelasticBase::compute_I2(const bool isPrevious) {
        ...
        std::array<T, dimension * dimension> C{};
    ...
    void HyperelasticBase::compute_dI2dFe(const bool isPrevious, dI2dFe_iter dI2dFe_begin, dI2dFe_iter dI2dFe_end) {
        ...
        TARDIGRADE_ERROR_TOOLS_CHECK((unsigned int)(dI2dFe_end - dI2dFe_begin) == dimension * dimension,
                                     "The size of dI2dFe is " +
                                         std::to_string((unsigned int)(dI2dFe_end - dI2dFe_begin)) +
                                         " but it must be " + std::to_string(dimension * dimension));
        ...
        std::array<dI2dFe_type, dimension * dimension> C{};
        compute_right_cauchy_green_deformation_tensor(isPrevious, std::begin(C), std::end(C));

        for (unsigned int a = 0; a < dimension; ++a) {
            for (unsigned int I = 0; I < dimension; ++I) {
                for (unsigned int A = 0; A < dimension; ++A) {
                    *(dI2dFe_begin + dimension * a + A) -= (*Fe)[dimension * a + I] * C[dimension * I + A];
                }
            }
        }
    ...
    void HyperelasticBase::compute_d2Ibar1dFe2(const bool isPrevious, d2Ibar1dFe2_iter d2Ibar1dFe2_begin,
        ...
        for (unsigned int aA = 0; aA < dimension * dimension; ++aA) {
            for (unsigned int bB = 0; bB < dimension * dimension; ++bB) {
                *(d2Ibar1dFe2_begin + dimension * dimension * aA + bB) +=
                    (d2I1dFe2[dimension * dimension * aA + bB] - f / (*Je) * (*dJedFe)[aA] * dI1dFe[bB] +
                     f * I1 / ((*Je) * (*Je)) * (*dJedFe)[aA] * (*dJedFe)[bB] -
                     f * I1 / (*Je) * (*d2JedFe2)[dimension * dimension * aA + bB]) /
                        std::pow((*Je), f) -
    ...
    void HyperelasticBase::compute_d2Ibar2dFe2(const bool isPrevious, d2Ibar2dFe2_iter d2Ibar2dFe2_begin,
        ...
        for (unsigned int aA = 0; aA < dimension * dimension; ++aA) {
            for (unsigned int bB = 0; bB < dimension * dimension; ++bB) {
                *(d2Ibar2dFe2_begin + dimension * dimension * aA + bB) +=
                    (d2I2dFe2[dimension * dimension * aA + bB] - f / (*Je) * (*dJedFe)[aA] * dI2dFe[bB] +
                     f * I2 / ((*Je) * (*Je)) * (*dJedFe)[aA] * (*dJedFe)[bB] -
                     f * I2 / (*Je) * (*d2JedFe2)[dimension * dimension * aA + bB]) /
                        std::pow((*Je), f) -
